...
const method_not_allowed = (method: string, pathname: string) =>
	Result.failure(new Response(`Method Not Allowed: "${method}" for path "${pathname}"`, { status: 405 }))
type MethodNotAllowedResult = FailureResult<Response>

type RouterFunction<Context, SuccessValue, FailureValue, InterruptValue> = (
	context: Context,
